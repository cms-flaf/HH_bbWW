...
def RecoHWWCandidateSelection(df):
    ...
    df = df.Define("Electron_iso", "Electron_miniPFRelIso_all").Define(
        "Muon_iso", "Muon_miniPFRelIso_all"
    )
...
def RecoHWWJetSelection(df):
    ...
    df = df.Define(
        "Jet_cleaned",
        " RemoveOverlaps(Jet_p4, Jet_sel,{ {FatJet_p4[FatJet_sel][0], },}, 1, 0.8)",
    )

    df = df.Define(
        "n_eff_Jets", "(FatJet_p4[FatJet_sel].size()*2)+(Jet_p4[Jet_cleaned].size())"
    )
...
def ExtraRecoJetSelection(df):
    df = df.Define(
        "ExtraJet_presel",
        f"v_ops::pt(Jet_p4)>20 && abs(v_ops::eta(Jet_p4)) < 5 && ( Jet_jetId & 2 )",
    )
    df = df.Define(
        "ExtraJet_noLep",
        "RemoveOverlaps(Jet_p4, ExtraJet_presel, HwwCandidate.getLegP4s(), 0.4)",
    )
    df = df.Define(
        "ExtraJet_sel",
        """auto sel = ExtraJet_noLep;
...
def DefineHbbCand(df):
    df = df.Define(
        "HbbCandidate", "GetHbbCandidate(Jet_btagPNetB, Jet_sel, Jet_p4, Jet_idx)"
    )
    return df
