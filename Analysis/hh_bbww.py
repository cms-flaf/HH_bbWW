...
class DataFrameBuilderForHistograms(DataFrameBuilderBase):
    ...
    def defineCategories(self):
        self.DefineAndAppend("nSelBtag_jets", f"int(bjet1_btagPNetB >= {self.bTagWP}) + int(bjet2_btagPNetB >= {self.bTagWP})")
        self.DefineAndAppend("nSelBtag_fatjets", f"int( SelectedFatJet_particleNet_XbbVsQCD[0] >= 0.8 )")
        self.DefineAndAppend("resolved", f"centralJet_pt.size() >= 2")
        self.DefineAndAppend("res2b", f"resolved && nSelBtag_jets >= 2")
        self.DefineAndAppend("boosted", f"!res2b && nSelBtag_fatjets > 0 ") # Greater than zero, but logic should only allow 0 or 1
        self.DefineAndAppend("res1b", f"SelectedFatJet_pt.size() == 0 && resolved && nSelBtag_jets == 1")
    ...
    def defineJetSelections(self):
        ...
        self.df = self.df.Define("fatsubjet1_isvalid","(SelectedFatJet_SubJet1_isValid == 1  && SelectedFatJet_SubJet1_pt > 20 && abs(SelectedFatJet_SubJet1_eta) < 2.5)")
        self.df = self.df.Define("fatsubjet2_isvalid","(SelectedFatJet_SubJet2_isValid == 1 && SelectedFatJet_SubJet2_pt > 20 && abs(SelectedFatJet_SubJet2_eta) < 2.5)")
        ...
        self.df = self.df.Define("bsubjet1_btagDeepB", "fatjet_isvalid ? SelectedFatJet_SubJet1_btagDeepB[0] : -1.0") #needs to be updated for ak8 PNet
        self.df = self.df.Define("bsubjet2_btagDeepB", "fatjet_isvalid ? SelectedFatJet_SubJet2_btagDeepB[0] : -1.0") #needs to be updated for ak8 PNet


    def defineQCDRegions(self):
        self.DefineAndAppend("OS", "(lep2_legType < 1) || (lep1_charge*lep2_charge < 0)")
    ...
    def defineControlRegions(self):
        ...
        self.DefineAndAppend("Zpeak", f"(lep1_legType == lep2_legType ) && (abs(diLep_mass - 91.1876) < 10)")
        self.DefineAndAppend("Zveto", f"(lep1_legType == lep2_legType ) && (abs(diLep_mass - 91.1876) > 10)")
        self.DefineAndAppend("OppFlavor", f"(lep1_legType != lep2_legType)")
        self.DefineAndAppend("TTbar_CR", f"OS_Iso && lep1_legType == lep2_legType && diLep_mass > 100 ")
        self.DefineAndAppend("mbb_SR", f"bb_mass_PNetRegPtRawCorr_PNetRegPtRawCorrNeutrino > 70 && bb_mass_PNetRegPtRawCorr_PNetRegPtRawCorrNeutrino < 150")
        self.DefineAndAppend("Lep1Lep2Jet1Jet2_mass", f"(lep1_legType == 2 && lep2_legType == 2) ? Lep1Lep2Jet1Jet2_p4.mass() : 0.0")
        self.DefineAndAppend("Lep1Jet1Jet2_mass", f"(lep1_legType == 2) ? Lep1Jet1Jet2_p4.mass() : 0.0")
    ...
    def __init__(self, df, config, period, colToSave=[], **kwargs):
        ...
        self.bTagWP = WorkingPointsParticleNet[period]["Medium"] #wp should go to global config.
