...
def main():
    parser = argparse.ArgumentParser(
        prog="check_all", description="Checks purity of the given sample"
    )
    ...
    parser.add_argument(
        "file_name", type=str, default="nano_0.root", help="Name of the .root file"
    )
    parser.add_argument(
        "sort_by", type=str, help="Name of the branch used to sort and select jets"
    )
    ...
    with open(cfg_name, "r") as cfg:
        ...
        for res in resonances:
            for channel_id, channel_name in channels.items():
                data = [
                    pc.ComputePurity(p, channel_name)
                    for p in paths
                    if channel_id in p and res in p
                ]
                output_name = (
                    f"out_{channel_name}_{res}_{tagger_name.split('-')[-1]}.csv"
                )
