...
def PlotMetric(history, model, metric, output_folder):
    ...
    plt.grid(True)
    plt.savefig(
        os.path.join(output_folder, f"{metric}_{model}.pdf"), bbox_inches="tight"
    )
    plt.clf()
...
class DataWrapper:
    ...
    def ReadFile(self, file_name, entry_start=None, entry_stop=None):
        ...
        self.param_values = np.array(
            [
                [
                    x if (x > 0) else np.random.choice(self.param_list)
                    for x in getattr(branches, "X_mass")
                ]
            ]
        ).transpose()
    ...
    def DefineTrainTestSet(self, batch_size, ratio):
        ...
        self.train_adv_regression_target = self.adv_regression_target[
            trainStart:trainEnd
        ]
        ...
        self.test_adv_regression_target = self.adv_regression_target[
            trainStart:trainEnd
        ]
    ...
    def monitor_param(self, model, param_value):
        ...
        arglist = None
        if param_value <= 0:
            arglist = self.features
        else:
            arglist = self.features_paramSet
        ...
        if param_value > 0:
            ...
            signal_mask = (self.labels != 1) | (
                self.param_values == param_value
            ).transpose()[0]
        ...
        cm = sklearn.metrics.confusion_matrix(
            self.labels, np.argmax(pred, axis=1), normalize="true"
        )
        ...
        for idx, c_label in enumerate(self.label_names):
            fpr, tpr, thresholds = sklearn.metrics.roc_curve(
                tf.keras.utils.to_categorical(self.labels, 3)[:, idx].astype(int),
                pred[:, idx],
            )
            c_ax.plot(
                fpr,
                tpr,
                label="{label} (AUC: {auc})".format(
                    label=c_label, auc=sklearn.metrics.auc(fpr, tpr)
                ),
            )
    ...
    def validate_output(self, sess, model_name, parity_index):
        ...
        for para_masspoint in para_masspoint_list:
            ...
            features = (
                self.features_paramSet
                if self.use_parametric
                else self.features_no_param
            )
            ...
            adv_loss_vec = binary_focal_crossentropy(
                tf.cast(self.adv_target, dtype=tf.float32),
                tf.cast(pred[1], dtype=tf.float32),
                tf.cast(tf.one_hot(self.class_target, nClasses), dtype=tf.float32),
            ...
            class_accuracy = round(
                np.average(class_accuracy_vec, weights=class_weight), 3
            )
            ...
            for process_name in mask_dict.keys():
                ...
                tmp_rootfile = uproot.recreate(
                    f"{process_name}_AdvOutput_par{parity_index}_M{para_masspoint}.root"
                )
                ...
                ROOT_ClassOutput_CR_high = ROOT.TH1D(
                ...
                for binnum in range(nQuantBins):
                    ROOT_ClassOutput_SR.SetBinContent(
                        binnum + 1, class_out_hist_SR[binnum]
                    )
                ...
                ROOT_ClassOutput_CR_high.Scale(
                    1.0 / ROOT_ClassOutput_CR_high.Integral()
                )
                ...
                max_val = max(
                    ROOT_ClassOutput_SR.GetMaximum(),
                    ROOT_ClassOutput_CR_high.GetMaximum(),
                )
                ROOT_ClassOutput_SR.GetYaxis().SetRangeUser(
                    0.1 * min_val, 20
                )  # 1000*max_val)
                ...
                legend.AddEntry(
                    ROOT_ClassOutput_CR_high, f"{process_name} m_bb CR High"
                )
                legend.Draw()
                ...
                plot_name = os.path.join(
                    save_path,
                    f"{process_name}_ClassOutput_par{parity_index}_M{para_masspoint}.pdf",
                )
                ...
                for binnum in range(nQuantBins):
                    ...
                    ROOT_ClassOutput_SR.SetBinError(
                        binnum + 1, class_out_hist_SR[binnum] ** (0.5)
                    )
                    ...
                    ROOT_ClassOutput_CR_high.SetBinError(
                        binnum + 1, class_out_hist_CR_high[binnum] ** (0.5)
                    )
                ...
                canvas.SaveAs(
                    os.path.join(
                        save_path,
                        f"{process_name}_ClassOutput_par{parity_index}_M{para_masspoint}_raw.pdf",
                    )
                )
                canvas.Close()
                ...
                min_val = max(
                    0.0001,
                    min(
                        ROOT_AdvOutput_SR.GetMinimum(),
                        ROOT_AdvOutput_CR_high.GetMinimum(),
                    ),
                )
                max_val = max(
                    ROOT_AdvOutput_SR.GetMaximum(), ROOT_AdvOutput_CR_high.GetMaximum()
                )
                ROOT_AdvOutput_SR.GetYaxis().SetRangeUser(
                    0.8 * min_val, 20
                )  # 1.5*max_val)
                ...
                chi2_value = ROOT_AdvOutput_SR.Chi2Test(
                    ROOT_AdvOutput_CR_high, option="WW"
                )
                ...
                plot_name = os.path.join(
                    save_path,
                    f"{process_name}_AdvOutput_par{parity_index}_M{para_masspoint}.pdf",
                )
                canvas.SaveAs(plot_name)
                canvas.Close()
                os.system(f"imgcat {plot_name}")
                ...
                for binnum in range(nQuantBins):
                    ROOT_AdvOutput_SR.SetBinContent(binnum + 1, adv_out_hist_SR[binnum])
                    ROOT_AdvOutput_SR.SetBinError(
                        binnum + 1, adv_out_hist_SR[binnum] ** (0.5)
                    )
                    ...
                    ROOT_AdvOutput_CR_high.SetBinError(
                        binnum + 1, adv_out_hist_CR_high[binnum] ** (0.5)
                    )
                ...
                canvas.SaveAs(
                    os.path.join(
                        save_path,
                        f"{process_name}_AdvOutput_par{parity_index}_M{para_masspoint}_raw.pdf",
                    )
                )
                canvas.Close()
...
def adv_regression_loss(target, output):
    ...
    clipped_target = tf.clip_by_value(
        target, clip_value_min=mbb_min, clip_value_max=mbb_max
    )
    ...
    random_indices = tf.random.uniform(
        shape=tf.shape(target), minval=0, maxval=tf.shape(target)[0], dtype=tf.int32
    )
    ...
    loss_random_realistic = tf.math.log(
        tf.cosh(diff_random)
    )  # This is a more stable loss function for regression
...
class AdversarialModelRegression(tf.keras.Model):
    ...
    def __init__(self, setup, *args, **kwargs):
        ...
        self.class_output = tf.keras.layers.Dense(
            2, activation="softmax", name="class_output"
        )
    ...
    def _step(self, data, training):
        ...
        def compute_losses():
            ...
            adv_loss_vec, adv_loss_mbb_vec, adv_loss_random_vec = self.adv_loss(
                y_adv, y_pred_adv
            )
        ...
        self.adv_loss_tracker.update_state(adv_loss_vec, sample_weight=adv_weight)
        self.adv_loss_mbb_tracker.update_state(
            adv_loss_mbb_vec, sample_weight=adv_weight
        )
        self.adv_loss_random_tracker.update_state(
            adv_loss_random_vec, sample_weight=adv_weight
        )
        ...
        if training:
            ...
            # grad_adv = adv_tape.gradient(adv_loss, common_vars + adv_vars)
            grad_adv = adv_tape.gradient(
                adv_loss_mbb_vec, common_vars + adv_vars
            )  # Don't minimize the final_loss, just try to minimize the true part
            grad_adv_excl = grad_adv[n_common_vars:]
    ...
    def _step_adv_only(self, data, training):
        ...
        def compute_losses(x_common):
            ...
            adv_loss_vec, adv_loss_random_vec = self.adv_loss(
                y_adv, y_pred_adv
            )  # Focal loss
            # adv_loss_vec = self.adv_loss(y_adv, y_pred_adv)
            # We want to apply some weights onto the adv loss vector
...
def train_dnn(setup, input_folder, output_folder, config_dict, val_config_dict):
    ...
    input_file_name = os.path.join(
        input_folder, config_dict["meta_data"]["input_filename"]
    )
    input_weight_name = os.path.join(
        input_folder, f"weight{config_dict['meta_data']['input_filename'][5:]}"
    )

    input_file_name_val = os.path.join(
        input_folder, val_config_dict["meta_data"]["input_filename"]
    )
    input_weight_name_val = os.path.join(
        input_folder, f"weight{val_config_dict['meta_data']['input_filename'][5:]}"
    )
    ...
    dw_val.ReadWeightFile(
        input_weight_name_val, entry_start=entry_start, entry_stop=entry_stop
    )
    ...
    val_tf_dataset = tf.data.Dataset.from_tensor_slices(
        (
            dw_val.features,
            (
                tf.one_hot(dw_val.class_target, nClasses),
                dw_val.adv_target,
                dw_val.class_weight,
                dw_val.adv_weight,
            ),
    ...
    val_tf_dataset = val_tf_dataset.shuffle(
        len(val_tf_dataset), reshuffle_each_iteration=True
    )
...
def train_regression_dnn(
    setup, input_folder, output_folder, config_dict, val_config_dict
):
    batch_size = config_dict["meta_data"]["batch_dict"]["batch_size"]
    val_batch_size = val_config_dict["meta_data"]["batch_dict"]["batch_size"]
...
def adv_only_training(
    ...
    model2.compile(
        loss=None,
        optimizer=tf.keras.optimizers.Nadam(
            learning_rate=setup2["learning_rate"], weight_decay=setup2["weight_decay"]
        ),
    )
...
def validate_model(
    model_name, model_config, validation_file, validation_weight, nParity
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="nParity number to train on for this call, default runs all",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        default="default_training_setup_doubleLep.yaml",
        help="Setup config for training",
    )
    ...
    yaml_list = [
        ...
        if fname.startswith("batch_config_parity")
    ]
        ...
        for i, config_yaml in enumerate(yaml_list):
            ...
            for j in range(4):
                ...
                model_name = os.path.join(
                    output_folder, f"ResHH_Classifier_parity{i}", "best.keras"
                )
                ...
                model_name = os.path.join(
                    output_folder, f"ResHH_Classifier_parity{i}", "best.onnx"
                )
                validate_model(model_name, model_config, test_file, test_weight, j)
                if setup["do_step2"]:
                    model_name = os.path.join(
                        output_folder, f"ResHH_Classifier_parity{i}", f"best_step2.onnx"
                    )
                    validate_model(model_name, model_config, test_file, test_weight, j)
