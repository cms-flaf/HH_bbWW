...
class DataWrapper:
    ...
    def get_generator(self, file_name, weight_file_name, batch_size):
        ...
        while entry_stop < tree.num_entries:
            ...
            if self.use_parametric:
                gen_features = np.append(gen_features, gen_param_values, axis=1)
    ...
    def SetMbbRegion(self, branches):
        ...
        mbb_region = np.where(mbb < 70, -1, np.where(mbb < 150, 0, 1))

        mbb_region_binary = np.where((mbb_region == 0), 1, 0)
    ...
    def validate_output(self, sess, model_name, parity_index):
        ...
        for para_masspoint in para_masspoint_list:
            ...
            adv_loss_vec = binary_focal_crossentropy(
                ...
                tf.cast(tf.one_hot(self.class_target, 2), dtype=tf.float32),
...
def train_dnn(setup, input_folder, output_folder, config_dict, val_config_dict):
    ...
    val_tf_dataset = tf.data.Dataset.from_tensor_slices(
        (
            dw_val.features,
            (
                tf.one_hot(dw_val.class_target, 2),
                dw_val.adv_target,
                dw_val.class_weight,
                dw_val.adv_weight,
            ),
