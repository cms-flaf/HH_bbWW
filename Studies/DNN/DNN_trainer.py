...
class DataWrapper:
    ...
    def ReadFile(self, file_name):
        ...
        self.labels = np.array(
            [
                self.value_to_label[str(branch_val)]
                for branch_val in labels_branch_values
            ]
        )
        print("Got labels")
    ...
    def validate_output(self, model):
        ...
        for para_masspoint in self.param_list:
            ...
            if para_masspoint > 1000:
                continue
            if para_masspoint not in [300, 450, 550, 800]:
                continue
            predict_list = []
            weight_list = []
            ...
            for i, label_name in enumerate(self.label_names):
                ...
                if i == 0:
                    ...
                    for real_mass in self.param_list:
                        ...
                        this_mass_mask = (
                            self.param_values[self.labels == i] == real_mass
                        )
            ...
            plt.yscale("log")
            plt.savefig(
                os.path.join(self.output_folder, f"dnn_values_M{para_masspoint}.pdf")
            )
            plt.clf()
            plt.close()
...
def train_dnn():
    ...
    history = model.fit(
        dw.train_features,
        dw.train_labels,
        validation_data=(dw.test_features, dw.test_labels),
        verbose=1,
        batch_size=900,
        epochs=10,
    ...
    with open(
        os.path.join(dw.output_folder, "dnn_config.yaml"),
        "w",
    ) as file:
