...
def create_weight_file(inName, outName, bb_low=70, bb_high=150, bb_min=70, bb_max=300):
    ...
    for nParity in range(4):
        print(f"On file {inName.format(nParity = nParity)}")
        in_file = uproot.open(inName.format(nParity = nParity))
        ...
        class_weight = np.where(
            sample_name == 'Signal',
            np.where(
                (hadronFlavour[:,0] == 5) & (hadronFlavour[:,1] == 5), # & (X_mass == 800), # For now, only train on m450
                class_weight,
                0.0
            ),
            class_weight
