...
class DataWrapper():
    ...
    def ReadFile(self, file_name, entry_start=None, entry_stop=None):
        ...
        print(f"Going to open file. Memory usage in MB is {psutil.Process(os.getpid()).memory_info()[0] / float(2 ** 20)}")
        ...
        with uproot.open(file_name) as file:
          ...
          print(f"Loaded branches. Memory usage in MB is {psutil.Process(os.getpid()).memory_info()[0] / float(2 ** 20)}")
          ...
          default_value = 0.0
          if self.listfeature_names != None: 
              self.listfeatures = np.array([ak.fill_none(ak.pad_none(getattr(branches, feature_name), index+1), default_value)[:,index] for [feature_name,index] in self.listfeature_names],dtype="float32").transpose()
          ...
          if self.highlevelfeatures_names != None: 
              self.hlv = np.array([getattr(branches, feature_name) for feature_name in self.highlevelfeatures_names],dtype="float32").transpose()
              self.features = np.append(self.features, self.hlv, axis=1)

          print(f"Set Features. Memory usage in MB is {psutil.Process(os.getpid()).memory_info()[0] / float(2 ** 20)}")
          ...
          self.param_values = np.array([getattr(branches, 'X_mass')],dtype="float32").transpose() # Init wrong parametric, later we will fill with random sample
        ...
        print(f"End read. Memory usage in MB is {psutil.Process(os.getpid()).memory_info()[0] / float(2 ** 20)}")
    ...
    def ReadWeightFile(self, weight_name, entry_start=None, entry_stop=None):
        ...
        with uproot.open(weight_name) as file:
          ...
          self.class_weight = np.array(getattr(branches, 'class_weight'), dtype="float32")
          self.adv_weight = np.array(getattr(branches, 'adv_weight'), dtype="float32")
          self.class_target = np.array(getattr(branches, 'class_target'), dtype="float32")
...
def distance_corr(var_1, var_2, normedweight, power=1):
    ...
    if power==1:
        dCorr = tf.reduce_mean(ABavg*normedweight)/tf.math.sqrt(tf.abs(tf.reduce_mean(AAavg*normedweight)*tf.reduce_mean(BBavg*normedweight))+1e-12)
    elif power==2:
        dCorr = (tf.reduce_mean(ABavg*normedweight))**2/(tf.abs(tf.reduce_mean(AAavg*normedweight)*tf.reduce_mean(BBavg*normedweight))+1e-12)
    else:
        dCorr = (tf.reduce_mean(ABavg*normedweight)/tf.math.sqrt(tf.reduce_mean(AAavg*normedweight)*tf.reduce_mean(BBavg*normedweight))+1e-12)**power
    ...
    if tf.math.is_nan(dCorr):
        ...
        tf.print(tf.reduce_mean(AAavg*normedweight)*tf.reduce_mean(BBavg*normedweight))
        tf.print(tf.math.sqrt(tf.reduce_mean(AAavg*normedweight)*tf.reduce_mean(BBavg*normedweight)))
        tf.print(dCorr)
        dCorr = tf.constant(0.0, dtype=tf.float32) # Can't use tf.where, maybe because it is only a single constant? or some reason
...
class ModelCheckpoint(tf.keras.callbacks.Callback):
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    if (self.predicate is None or self.predicate(self.model, logs)): # Save all for now
      ...
      if os.path.exists(path_best):
        os.remove(path_best)
        if os.path.exists(path_best_keras): os.remove(path_best_keras)
...
class AdversarialModel(tf.keras.Model):
  ...
  def __init__(self, setup, *args, **kwargs):
    ...
    self.other_class_min_tracker = [ tf.keras.metrics.Mean(name=f"other_class_min{n}") for n in range(self.nClasses) if n != 0 ]
    self.other_class_max_tracker = [ tf.keras.metrics.Mean(name=f"other_class_max{n}") for n in range(self.nClasses) if n != 0 ]


    self.common_layers = []

    def add_layer(layer_list, n_units, activation, name):
      layer = tf.keras.layers.Dense(n_units, activation=activation, name=name, kernel_initializer='random_normal', bias_initializer='random_normal')
  ...
  def _step(self, data, training):
    ...
    disco_weight = tf.expand_dims(tf.cast(y[0][:,0] == 0, dtype=tf.float32), axis=-1)

    def compute_losses():
      y_pred_class, y_pred_adv = self(x, training=training)

      class_loss_vec = self.class_loss(y_class, y_pred_class)
      disco_loss = self.lambda_disco * disco_weight * self.disco_loss(y_pred_class[:,0], y_adv, adv_weight)
      ...
      return y_pred_class, class_loss_vec, class_loss, y_pred_adv, adv_loss_vec, adv_loss, disco_loss

    if training:
      with tf.GradientTape() as class_tape, tf.GradientTape() as adv_tape:
        y_pred_class, class_loss_vec, class_loss, y_pred_adv, adv_loss_vec, adv_loss, disco_loss = compute_losses()
    else:
      y_pred_class, class_loss_vec, class_loss, y_pred_adv, adv_loss_vec, adv_loss, disco_loss = compute_losses()
    ...
    for n in range(self.nClasses):
       if n == 0: continue
       self.other_class_min_tracker[n-1].update_state(tf.reduce_min(y_pred_class[:,n]))
       self.other_class_max_tracker[n-1].update_state(tf.reduce_max(y_pred_class[:,n]))
    ...
    if training:
      ...
      grad_common_growing_adv = [ self.class_grad_factor * grad_class[i] - (self.adv_grad_factor + (0.1 * self.epoch_counter)) * grad_adv[i] \
      ...
      grad_common_with_adv = [ self.class_grad_factor * grad_class[i] + self.adv_grad_factor * grad_adv[i] \
                      for i in range(len(common_vars)) ]

      grad_common_only_neg_adv = [ (1.0) * grad_adv[i] \
                      for i in range(len(common_vars)) ]
      ...
      def apply_common_only_neg_adv():
        self.optimizer.apply_gradients(zip(grad_common_only_neg_adv, common_vars))
        return 
  ...
  def metrics(self):
    ...
    metric_list = metric_list + [ x for x in self.other_class_min_tracker] + [ x for x in self.other_class_max_tracker]
...
class DiscoModel(tf.keras.Model):
  ...
  def __init__(self, setup, *args, **kwargs):
    ...
    for n in range(setup['n_disco_layers']):
      add_layer(self.disco_layers, setup['n_disco_units'], setup['disco_activation'], f'disco_{n}')
  ...
  def _step(self, data, training):
    ...
    def compute_losses():
      ...
      class_loss_vec = self.class_loss(y_class, y_pred_class)
      disco_loss = self.lambda_disco * disco_weight * self.disco_loss(mbb, y_pred_class[:,0], class_weight, 2)
    ...
    if training:
      with tf.GradientTape() as class_tape:
        y_pred_class, class_loss_vec, class_loss, disco_loss, combined_loss = compute_losses()
    else:
      y_pred_class, class_loss_vec, class_loss, disco_loss, combined_loss = compute_losses()
...
def train_dnn(setup, training_file, weight_file, config_dict, test_training_file, test_weight_file, test_config_dict, output_folder):
  ...
  # Disco Dataset
  nClasses = setup['nClasses']
  train_tf_dataset = tf.data.Dataset.from_tensor_slices((dw.features, (tf.one_hot(dw.class_target, nClasses), dw.mbb, dw.class_weight))).batch(batch_size, drop_remainder=True)
  train_tf_dataset = train_tf_dataset.shuffle(len(train_tf_dataset), reshuffle_each_iteration=True)

  test_tf_dataset = tf.data.Dataset.from_tensor_slices((test_dw.features, (tf.one_hot(test_dw.class_target, nClasses), test_dw.mbb, test_dw.class_weight))).batch(test_batch_size, drop_remainder=True)
  test_tf_dataset = test_tf_dataset.shuffle(len(test_tf_dataset), reshuffle_each_iteration=True)
  ...
  callbacks = [
      ModelCheckpoint(output_dnn_name, verbose=1, monitor="val_class_loss", mode='min', min_rel_delta=1e-3,
                      patience=100, save_callback=None, input_signature=input_signature),
  ]
...
def validate_dnn(setup, validation_file, validation_weight_file, config_dict, output_file, model_name, model_config):
    ...
    for para_masspoint in para_masspoint_list:
      if dw.use_parametric: dw.SetPredictParamValue(para_masspoint)
      ...
      nClasses = setup['nClasses']
      adv_loss_vec = binary_focal_crossentropy(tf.cast(dw.adv_target, dtype=tf.float32), tf.cast(pred[1], dtype=tf.float32), tf.cast(tf.one_hot(dw.class_target, nClasses), dtype=tf.float32), tf.cast(pred_class, dtype=tf.float32))
      adv_loss = round(np.average(adv_loss_vec, weights=adv_weight),3)
      adv_accuracy_vec = accuracy(tf.cast(dw.adv_target, dtype=tf.float32), tf.cast(pred[1], dtype=tf.float32))[:,0]
      adv_accuracy = round(np.average(adv_accuracy_vec, weights=adv_weight),3)

      class_loss_vec = categorical_crossentropy(tf.cast(tf.one_hot(dw.class_target, nClasses), dtype=tf.float32), tf.cast(pred_class, dtype=tf.float32))
      class_loss = round(np.average(class_loss_vec, weights=class_weight),3)
      class_accuracy_vec = categorical_accuracy(tf.cast(tf.one_hot(dw.class_target, nClasses), dtype=tf.float32), tf.cast(pred_class, dtype=tf.float32))
      ...
      for i, process_name in enumerate(mask_dict.keys()):
        ...
        Class_SR_list.append(ROOT.TH1D(f"ClassOutput_{process_name}_SR", f"ClassOutput_{process_name}_SR", nQuantBins, 0.0, 1.0))
        Class_CR_list.append(ROOT.TH1D(f"ClassOutput_{process_name}_CR_high", f"ClassOutput_{process_name}_CR_high", nQuantBins, 0.0, 1.0))
        ...
        if ROOT_ClassOutput_SR.Integral() == 0:
          print(f"Process {process_name} has no class entries, maybe the background doesn't exist?")
          continue
        ...
        Adv_SR_list.append(ROOT.TH1D(f"AdvOutput_{process_name}_SR", f"AdvOutput_{process_name}_SR", nQuantBins, 0.0, 1.0))
        Adv_CR_list.append(ROOT.TH1D(f"AdvOutput_{process_name}_CR_high", f"AdvOutput_{process_name}_CR_high", nQuantBins, 0.0, 1.0))
...
def validate_disco_dnn(setup, validation_file, validation_weight_file, config_dict, output_file, model_name, model_config):
    print(f"Model load {model_name}")
    sess = ort.InferenceSession(model_name)
