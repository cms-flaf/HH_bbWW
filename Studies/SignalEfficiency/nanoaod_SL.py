...
def process_tree_Gen(file_path, tree_name):
    ...
    if tree_name == "Events":
        ...
        df = df.Define(
            "Close0RecoLepIdx",
            f"FindMatching(H_to_VV.legs.at(0).leg_vis_p4.at(0), {mychfull}_p4, 1)",
        )
    ...
    if mychannel == "sle":
        ...
        df_Gen_Accept = df_GenSL.Filter(
            "GenLep0_p4.pt()>5 && abs (GenLep0_p4.eta()) <2.5", ""
        )
    ...
    df_GenWq_Accept = df_Genb_Accept.Filter(
        "H_to_VV.legs.at(1).leg_p4.at(0).pt()>20 && H_to_VV.legs.at(1).leg_p4.at(1).pt()>20 && abs (H_to_VV.legs.at(1).leg_p4.at(0).eta()) <5 && abs (H_to_VV.legs.at(1).leg_p4.at(1).eta()) <5",
        "",
    )
...
def EventsDistribution1D_hist(mass_point, df):
    ...
    df = df.Define(
        "FatJet_p4", "yumengGetP4(FatJet_pt, FatJet_eta, FatJet_phi, FatJet_mass)"
    )
    df = df.Define(
        "mindRGenlepFatjet",
        "FindMatchingdR(H_to_VV.legs.at(0).leg_vis_p4.at(0), FatJet_p4, 100)",
    )
    df = df.Define(
        "dRGenlepVh",
        "ROOT::Math::VectorUtil::DeltaR(H_to_VV.legs.at(0).leg_vis_p4.at(0), H_to_VV.legs.at(1).cand_p4)",
    )
    ...
    hist_pt = df.Histo1D(
        (
            f"mindR(GenLep, Fatjet){mass_point}",
            f"mindR(GenLep, Fatjet), SL{mychbrief} channel",
            50,
            0,
            0.8,
        ),
...
def EventsDistribution2D_hist(mass_point, df):
    ...
    df = df.Define(
        "dR_GenRecoLep",
        f"ROOT::Math::VectorUtil::DeltaR(GenLep0_p4, {mychfull}_p4.at(Close0RecoLepIdx))",
    )
    df = df.Define(
        "PtRatio_GenReco0Lep", f"GenLep0Pt / {mychfull}_pt.at(Close0RecoLepIdx)"
    )
    ...
    model_2d = ROOT.RDF.TH2DModel(
        ...
        f"{particle2} vs {particle} M-{mass_point}({mychannel})",
        binsx,
        pt_min,
        xmax,
        binsy,
...
def TEff_RecoGenMatch_hist(mass_point, df):
    ...
    df = df.Define(
        "mindRGenlepJet",
        "FindMatchingdR(H_to_VV.legs.at(0).leg_vis_p4.at(0), Jet_p4, 100)",
    )
    ...
    h_before = df.Histo1D(
        ("h_before", f"{var1} Distribution", bins, var_min, var_max), f"{particl}"
    ).GetValue()
    h_after = df_sz.Histo1D(
        ("h_after", f"HasRecoE_Eff vs {var1} (M-{mass_point})", bins, var_min, var_max),
        f"{particl}",
    ).GetValue()
    ...
    h_before.SetBinContent(
        bins, h_before.GetBinContent(bins) + h_before.GetBinContent(bins + 1)
    )
    h_after.SetBinContent(
        bins, h_after.GetBinContent(bins) + h_after.GetBinContent(bins + 1)
    )
    ...
    hist_pt.SetTitle(
        f"Has_e_Eff vs {var1} M-{mass_point};{var1}, SL{mychbrief} Channel;Eff(after HasRecoElectron)"
    )
...
def comparecut_hist(mass_point, df):
    ...
    hist = ROOT.TH1F(
        f"Relativre Eff for diff cuts{mass_point}",
        "Relative Eff for diff cuts",
        len(labels),
        0,
        len(labels),
    )
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if hist_type == "EventsDistribution1D":
            hist, yrange, fname = EventsDistribution1D_hist(
                mass_point, df_GenWqAccept_Eventstree
            )
        if hist_type == "EventsDistribution2D":
            hist, yrange, particle = EventsDistribution2D_hist(
                mass_point, df_GenSL_Eventstree
            )
            ROOT.gStyle.SetOptStat(0)
            hist.Draw("COLZ")
