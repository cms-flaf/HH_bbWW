...
def process_tree_Gen(file_path, tree_name):
    ...
    if tree_name == "Events":
        ...
        df = df.Define(
            "H_to_VV",
            """GetGenHVVCandidate(event, genLeptons, GenPart_pdgId, GenPart_daughters, 
        ...
        df = df.Define(
            f"{mychfull}_p4",
            f"yumengGetP4({mychfull}_pt, {mychfull}_eta, {mychfull}_phi, {mychfull}_mass)",
        )
        if mychannel == "dle" or mychannel == "dlmuon":
            for i in range(2):
                df = df.Define(
                    f"RecoLepIdx{i}",
                    f"FindMatching(H_to_VV.legs.at({i}).leg_vis_p4.at(0), {mychfull}_p4, {matchThresh})",
                )
        if mychannel == "dlmu_e":
            df = df.Define(
                f"{mychfull1}_p4",
                f"yumengGetP4({mychfull1}_pt, {mychfull1}_eta, {mychfull1}_phi, {mychfull1}_mass)",
            )
        ...
        df = df.Define(
            "H_to_VV",
            """GetGenHVVCandidate(-1,genLeptons, GenPart_pdgId, GenPart_daughters, 
                                  GenPart_statusFlags, GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, GenJet_p4, true)""",
        )
        df = df.Define(
            "H_to_BB",
            """GetGenHBBCandidate(-1, GenPart_pdgId, GenPart_daughters, GenPart_statusFlags, GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, GenJet_p4, true)""",
        )
    ...
    if mychannel == "dle":
        df_GenDL = df.Filter(
            "Leg0_Kind == Vleg::PromptElectron && Leg1_Kind == Vleg::PromptElectron", ""
        )
        genDL_count = get_event_counts(df_GenDL)

        df_Gen_Accept = df_GenDL.Filter(
            "GenLep0_p4.pt() > 5 && abs(GenLep0_p4.eta()) < 2.5 && GenLep1_p4.pt() > 5 && abs(GenLep1_p4.eta()) < 2.5",
            "",
        )
    ...
    if mychannel == "dlmuon":
        df_GenDL = df.Filter(
            "Leg0_Kind==Vleg::PromptMuon && Leg1_Kind==Vleg::PromptMuon", ""
        )
        genDL_count = get_event_counts(df_GenDL)
    ...
    if mychannel == "dlmu_e":
        ...
        df_GenDL = df_GenDL.Define(
            "GenMu_p4", "Leg0_Kind == Vleg::PromptMuon ? GenLep0_p4 : GenLep1_p4"
        ).Define(
            "Gene_p4", "Leg0_Kind == Vleg::PromptElectron ? GenLep0_p4 : GenLep1_p4"
        )

        df_GenDL = df_GenDL.Define(
            "RecoLepIdx0", f"FindMatching(GenMu_p4, Muon_p4, {matchThresh})"
        ).Define("RecoLepIdx1", f"FindMatching(Gene_p4, Electron_p4, {matchThresh1})")

        df_Gen_Accept = df_GenDL.Filter(
            "GenMu_p4.pt() > 5 && abs(GenMu_p4.eta()) < 2.5 && Gene_p4.pt() > 5 && abs(Gene_p4.eta()) < 2.5",
            "",
        )
        gen_accept_count = get_event_counts(df_Gen_Accept)

    df_Genb_Accept = df_Gen_Accept.Filter(
        "Genb0_p4.pt()>20 && Genb1_p4.pt()>20 && abs (Genb0_p4.eta()) <2.5 && abs (Genb1_p4.eta()) <2.5",
        "",
    )
...
def RecoHWWCandidateSelection(df):
    ...
    if mychannel == "dle":
        ...
        df = df.Filter(
            "RecoLepIdx0>=0 && RecoLepIdx1 >=0 && RecoLepIdx0!=RecoLepIdx1",
            "Reco_ee_MatchGen",
        )
        df = df.Filter(
            "Electron_pt.at(RecoLepIdx0)>5 && Electron_pt.at(RecoLepIdx1)>5",
            "Reco_ee_pt>5",
        )  # 10
        df = df.Filter(
            "Electron_eta.at(RecoLepIdx0)<2.5 && Electron_eta.at(RecoLepIdx1)<2.5",
            "Reco_ee_eta<2.5",
        )
        df = df.Filter(
            "Electron_dz.at(RecoLepIdx0)<0.1 && Electron_dz.at(RecoLepIdx1)<0.1",
            "Reco_ee_dz<0.1",
        )
        df = df.Filter(
            "Electron_dxy.at(RecoLepIdx0)<0.05 && Electron_dxy.at(RecoLepIdx1)<0.05",
            "Reco_ee_dxy<0.05",
        )
        df = df.Filter(
            "Electron_sip3d.at(RecoLepIdx0) <= 8 && Electron_sip3d.at(RecoLepIdx1) <= 8",
            "Reco_ee_sip3d <= 8",
        )
    ...
    if mychannel == "dlmuon":
        ...
        df = df.Filter(
            "Muon_pt.at(RecoLepIdx0)>5 && Muon_pt.at(RecoLepIdx1)>5", "Reco_mumu_pt>5"
        )  # 15
        df = df.Filter(
            "Muon_eta.at(RecoLepIdx0)<2.5 && Muon_eta.at(RecoLepIdx1)<2.5",
            "Reco_mumu_eta<2.5",
        )  # 2.4
        df = df.Filter(
            "Muon_dz.at(RecoLepIdx0)<0.1 && Muon_dz.at(RecoLepIdx1)<0.1",
            "Reco_mumu_dz<0.1",
        )
        df = df.Filter(
            "Muon_dxy.at(RecoLepIdx0)<0.05 && Muon_dxy.at(RecoLepIdx1)<0.05",
            "Reco_mumu_dxy<0.05",
        )
        df = df.Filter(
            "Muon_sip3d.at(RecoLepIdx0) <= 8 && Muon_sip3d.at(RecoLepIdx1) <= 8",
            "Reco_mumu_sip3d <= 8",
        )
    ...
    if mychannel == "dlmu_e":
        ...
        df = df.Filter(
            "Muon_pt.at(RecoLepIdx0)>5 && Electron_pt.at(RecoLepIdx1)>5",
            "Reco_emu_pt>5",
        )
        df = df.Filter(
            "Muon_eta.at(RecoLepIdx0)<2.5 && Electron_eta.at(RecoLepIdx1)<2.5",
            "Reco_emu_eta<2.5",
        )
        df = df.Filter(
            "Muon_dz.at(RecoLepIdx0)<0.1 && Electron_dz.at(RecoLepIdx1)<0.1",
            "Reco_emu_dz<0.1",
        )
        df = df.Filter(
            "Muon_dxy.at(RecoLepIdx0)<0.05 && Electron_dxy.at(RecoLepIdx1)<0.05",
            "Reco_emu_dxy<0.05",
        )
        df = df.Filter(
            "Muon_sip3d.at(RecoLepIdx0) <= 8 && Electron_sip3d.at(RecoLepIdx1) <= 8",
            "Reco_emu_sip3d <= 8",
        )
    return df


def Report_cutflow_hist(
    canvas,
    mass_point,
    report,
    initial_count,
    ...
    hist_eff = ROOT.TH1D(
        reportName + f"__M-{mass_point}" + "_Efficiency",
        reportName + f" {ctype}" + " Efficiency" + "_for Masspoints",
        num_cuts + 4,
        0,
        num_cuts + 4,
    )
...
def EventsDistribution1D_hist(mass_point, df):
    if mychannel == "dle" or mychannel == "dlmuon":
        for i in range(2):
            df = df.Define(
                f"RecolepdRSize{i}",
                f"FindMatchingSet(H_to_VV.legs.at({i}).leg_vis_p4.at(0), {mychfull}_p4, 0.02).size()",
            )
    if mychannel == "dlmu_e":
        df = df.Define(
            f"RecolepdRSize0", f"FindMatchingSet(GenMu_p4, Muon_p4, 0.02).size()"
        ).Define(
            f"RecolepdRSize1", f"FindMatchingSet(Gene_p4, Electron_p4, 0.02).size()"
        )
    ...
    hist_pt = df.Histo1D(
        (
            f"recolepdRSize-{mass_point}",
            f"Count of Recoleps within dR of Genlep({mychbrief})",
            10,
            0,
            10,
        ),
...
def EventsDistribution2D(mass_point, df):
    if mychannel == "dle" or mychannel == "dlmuon":
        for i in range(2):
            df = df.Define(
                f"dR_GenRecoLep{i}",
                f"ROOT::Math::VectorUtil::DeltaR(GenLep{i}_p4, {mychfull}_p4.at(RecoLepIdx{i}))",
            )
            df = df.Define(
                f"PtRatio_GenRecoLep{i}",
                f"GenLep{i}_p4.pt() / {mychfull}_pt.at(RecoLepIdx{i})",
            )
    if mychannel == "dlmu_e":
        df = df.Define(
            f"dR_GenRecoLep0",
            f"ROOT::Math::VectorUtil::DeltaR(GenMu_p4, Muon_p4.at(RecoLepIdx0))",
        ).Define(
            f"dR_GenRecoLep1",
            f"ROOT::Math::VectorUtil::DeltaR(Gene_p4, Electron_p4.at(RecoLepIdx1))",
        )
        df = df.Define(
            f"PtRatio_GenRecoLep0", f"GenMu_p4.pt() / Muon_pt.at(RecoLepIdx0)"
    ...
    model_2d = ROOT.RDF.TH2DModel(
        f"{particle2}_vs_{particle}_M-{mass_point}",
        f"{particle2} vs {particle} M{mass_point}({mychbrief})",
        binsx,
        pt_min,
        xmax,
        binsy,
...
def comparecut_hist(mass_point, df, initial_count):
    ...
    hist = ROOT.TH1F(
        f"Cumulative Eff for diff cuts{mass_point}",
        "Cumulative Eff for diff cuts",
        len(labels),
        0,
        len(labels),
    )
...
def main(mychannel, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        (
            df_GenDL_Eventstree,
            df_GenAccept_Eventstree,
            df_GenbAccept_Eventstree,
            genDL_count_eventstree,
            gen_accept_count_eventstree,
            genb_accept_count_eventstree,
        ) = process_tree_Gen(file_path, "Events")

        if hist_type == "cutflow":
            hist = process_cutflow_hist(
                canvas,
                mass_point,
                initial_count,
                genDL_count_eventstree,
        ...
        if hist_type == "EventsDistribution1D":
            hist, yrange, fname = EventsDistribution1D_hist(
                mass_point, df_GenbAccept_Eventstree
            )
        if hist_type == "EventsDistribution2D":
            hist, yrange, particle = EventsDistribution2D(
                mass_point, df_GenDL_Eventstree
            )
