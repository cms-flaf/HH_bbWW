...
namespace HME {
    inline Float_t mT(LorentzVectorF_t p) { return std::sqrt(p.M2() + p.Pt() * p.Pt()); }

    LorentzVectorF_t SamplePNetResCorr(LorentzVectorF_t const& jet,
    ...
    OptLVecF_t NuFromOffshellW(LorentzVectorF_t const& lep1,
        ...
        Float_t cosh_deta =
            (mh * mh + 2.0 * (nu_tmp_px * tmp.Px() + nu_tmp_py * tmp.Py()) - tmp.M2()) / (2.0 * tmp2.Pt() * pt);
        if (cosh_deta < 1.0) {
            return std::nullopt;
        }
    ...
    OptLVecFPair_t NuFromConstraints(LorentzVectorF_t const& jet1,
                                     LorentzVectorF_t const& jet2,
                                     LorentzVectorF_t const& lep,
                                     LorentzVectorF_t const& met,
                                     Float_t mw,
