...
for i, ev in enumerate(tqdm.tqdm(events)):
    ...
    if not (
        (ev.lep1_pt > 0)
        and (ev.lep2_pt > 0)
        and (ev.centralJet_pt[0] > 0)
        and (ev.centralJet_pt[1] > 0)
    ):
        continue
    ...
    jet1_p4.SetPtEtaPhiM(
        ev.centralJet_pt[0],
        ev.centralJet_eta[0],
        ev.centralJet_phi[0],
        ev.centralJet_mass[0],
    )
    jet2_p4.SetPtEtaPhiM(
        ev.centralJet_pt[1],
        ev.centralJet_eta[1],
        ev.centralJet_phi[1],
        ev.centralJet_mass[1],
    )
    ...
    if runHME:
        ...
        if hme.hme_h2Mass.GetEntries() <= 0:
            ...
            hme.hme_h2Mass.GetEntries() > 0
            and hme.hme_h2Mass.GetXaxis().GetBinCenter(hme.hme_h2Mass.GetMaximumBin())
            < 250.0
        ):
            print(
                "Num solutions ",
                hme.hme_h2Mass.GetEntries(),
                " BUT the maximum is ",
        ...
        if (
            ...
            hme_h2mass_reco = hme.hme_h2Mass.GetXaxis().GetBinCenter(
                hme.hme_h2Mass.GetMaximumBin()
            )
            ...
            hme_entry_peak_reco = hme.hme_h2Mass.Integral(
                hme.hme_h2Mass.GetMaximumBin() - 5, hme.hme_h2Mass.GetMaximumBin() + 5
            )

            hme_h2mass_weight1_reco = hme.hme_h2MassWeight1.GetXaxis().GetBinCenter(
                hme.hme_h2MassWeight1.GetMaximumBin()
            )
            hme_mean_weight1_reco = hme.hme_h2MassWeight1.GetMean()
            hme_stddev_weight1_reco = hme.hme_h2MassWeight1.GetStdDev(1)
            hme_entries_weight1_reco = (
                float(hme.hme_h2MassWeight1.GetEntries()) / iterations
            )
            hme_entry_peak_weight1_reco = hme.hme_h2MassWeight1.Integral(
                hme.hme_h2MassWeight1.GetMaximumBin() - 5,
                hme.hme_h2MassWeight1.GetMaximumBin() + 5,
            )

            hme_h2mass_weight2_reco = hme.hme_h2MassWeight2.GetXaxis().GetBinCenter(
                hme.hme_h2MassWeight2.GetMaximumBin()
            )
            ...
            hme_entries_weight2_reco = (
                float(hme.hme_h2MassWeight2.GetEntries()) / iterations
            )
            hme_entry_peak_weight2_reco = hme.hme_h2MassWeight2.Integral(
                hme.hme_h2MassWeight2.GetMaximumBin() - 5,
                hme.hme_h2MassWeight2.GetMaximumBin() + 5,
            )
            if hme.debug:
                print(
                    "True HH mass ",
                    truemass,
                    "; reconstructed HH mass ",
                    hme_h2mass_reco,
                    " +/- ",
                    hme_stddev_reco,
                )
                print(
                    "\t reconstructed HH mass with type 1 weight ",
                    hme_h2mass_weight1_reco,
                    " +/- ",
                    hme_stddev_weight1_reco,
                )
                print(
                    "\t reconstructed HH mass with type 2 weight ",
...
print(
    "T. Huang, J.M. No, L. Pernie,  M. Ramsey-Musolf, A. Safonov, M. Spannowsky, and P. Winslow                                               "
)
print(
