...
class HeavyMassEstimator(object):
    ...
    eta_gen = np.zeros(1, dtype=float)
    phi_gen = np.zeros(1, dtype=float)
    wmass_gen = np.zeros(1, dtype=float)
    hmass_gen = np.zeros(1, dtype=float)
    metpx_corr = np.zeros(1, dtype=float)
    metpy_corr = np.zeros(1, dtype=float)
    ...
    l_onshellW_eta = np.zeros(1, dtype=float)
    l_onshellW_phi = np.zeros(1, dtype=float)
    l_onshellW_pt = np.zeros(1, dtype=float)
    l_onshellW_energy = np.zeros(1, dtype=float)
    l_offshellW_eta = np.zeros(1, dtype=float)
    l_offshellW_phi = np.zeros(1, dtype=float)
    l_offshellW_pt = np.zeros(1, dtype=float)
    l_offshellW_energy = np.zeros(1, dtype=float)
    nu_onshellW_eta = np.zeros(1, dtype=float)
    nu_onshellW_phi = np.zeros(1, dtype=float)
    nu_onshellW_pt = np.zeros(1, dtype=float)
    nu_onshellW_energy = np.zeros(1, dtype=float)
    nu_offshellW_eta = np.zeros(1, dtype=float)
    nu_offshellW_phi = np.zeros(1, dtype=float)
    nu_offshellW_pt = np.zeros(1, dtype=float)
    nu_offshellW_energy = np.zeros(1, dtype=float)
    onshellW_eta = np.zeros(1, dtype=float)
    onshellW_phi = np.zeros(1, dtype=float)
    onshellW_pt = np.zeros(1, dtype=float)
    onshellW_energy = np.zeros(1, dtype=float)
    onshellW_mass = np.zeros(1, dtype=float)
    offshellW_eta = np.zeros(1, dtype=float)
    offshellW_phi = np.zeros(1, dtype=float)
    offshellW_pt = np.zeros(1, dtype=float)
    offshellW_energy = np.zeros(1, dtype=float)
    offshellW_mass = np.zeros(1, dtype=float)
    met_pt = np.zeros(1, dtype=float)
    met_phi = np.zeros(1, dtype=float)
    met_px = np.zeros(1, dtype=float)
    met_py = np.zeros(1, dtype=float)
    b1jet_eta = np.zeros(1, dtype=float)
    b1jet_phi = np.zeros(1, dtype=float)
    b1jet_pt = np.zeros(1, dtype=float)
    b1jet_energy = np.zeros(1, dtype=float)
    b2jet_eta = np.zeros(1, dtype=float)
    b2jet_phi = np.zeros(1, dtype=float)
    b2jet_pt = np.zeros(1, dtype=float)
    b2jet_energy = np.zeros(1, dtype=float)
    b1rescalefactor = np.zeros(1, dtype=float)
    b2rescalefactor = np.zeros(1, dtype=float)
    htoBB_eta = np.zeros(1, dtype=float)
    htoBB_phi = np.zeros(1, dtype=float)
    htoBB_pt = np.zeros(1, dtype=float)
    htoBB_energy = np.zeros(1, dtype=float)
    htoWW_eta = np.zeros(1, dtype=float)
    htoWW_phi = np.zeros(1, dtype=float)
    htoWW_pt = np.zeros(1, dtype=float)
    htoWW_energy = np.zeros(1, dtype=float)
    htoBB_mass = np.zeros(1, dtype=float)
    htoWW_mass = np.zeros(1, dtype=float)
    h2tohh_eta = np.zeros(1, dtype=float)
    h2tohh_phi = np.zeros(1, dtype=float)
    h2tohh_pt = np.zeros(1, dtype=float)
    h2tohh_energy = np.zeros(1, dtype=float)
    h2tohh_mass = np.zeros(1, dtype=float)
    h2tohh_mass_total = np.zeros(1, dtype=float)
    ...
    def __init__(self):
        ...
        nbinsMass = 3800
        self.debug = False
        self.hme_h2Mass = ROOT.TH1F(
            "hme_h2Mass", "h2 mass from HME", nbinsMass, minMass, maxMass
        )
        self.hme_h2Mass_divSolutions = ROOT.TH1F(
            "hme_h2Mass_divSolutions",
            "h2 mass from HME / nSolutions",
            nbinsMass,
            minMass,
            maxMass,
        )
        self.hme_h2Mass_correctmunupair = ROOT.TH1F(
            "hme_h2Mass_correctmunupair",
            "h2 mass from HME",
            nbinsMass,
            minMass,
            maxMass,
        )
        self.hme_h2Mass_incorrectmunupair = ROOT.TH1F(
            "hme_h2Mass_incorrectmunupair",
            "h2 mass from HME",
            nbinsMass,
            minMass,
            maxMass,
        )
        self.hme_h2MassWeight1 = ROOT.TH1F(
            "hme_h2MassWeight1", "h2 mass from HME", nbinsMass, minMass, maxMass
        )
        self.hme_h2MassWeight2 = ROOT.TH1F(
            "hme_h2MassWeight2", "h2 mass from HME", nbinsMass, minMass, maxMass
        )
        self.hme_h2MassWeight3 = ROOT.TH1F(
            "hme_h2MassWeight3", "h2 mass from HME", nbinsMass, minMass, maxMass
        )
        # self.hme_h2MassWeight4 = ROOT.TH1F("hme_h2MassWeight4","h2 mass from HME",nbinsMass, minMass, maxMass)
        self.hme_offshellWmass = ROOT.TH1F(
            "hme_offshellWmass", "offshell W mass from HME", 100, 0.0, 100.0
        )
        ### offshell Wmass(y-axis) Vs HME
        self.hme_h2MassAndoffshellWmass = ROOT.TH2F(
            "hme_h2MassAndoffshellWmass",
            "h2 Mass and offshell W mass from HME",
            nbinsMass,
            minMass,
            maxMass,
            100,
        ...
        self.hme_h2MassAndoffshellWmass_correctmunupair = ROOT.TH2F(
            "hme_h2MassAndoffshellWmass_correctmunupair",
            "h2 Mass and offshell W mass from HME",
            nbinsMass,
            minMass,
            maxMass,
        ...
        self.hme_h2MassAndoffshellWmass_weight1 = ROOT.TH2F(
            "hme_h2MassAndoffshellWmass_weight1",
            "h2 Mass and offshell W mass from HME",
            nbinsMass,
            minMass,
            maxMass,
        ...
        self.hme_h2MassAndoffshellWmass_weight2 = ROOT.TH2F(
            "hme_h2MassAndoffshellWmass_weight2",
            "h2 Mass and offshell W mass from HME",
            nbinsMass,
            minMass,
            maxMass,
        ...
        self.hmetree.Branch(
            "l_onshellW_energy", self.l_onshellW_energy, "l_onshellW_energy/D"
        )
        self.hmetree.Branch(
            "l_offshellW_eta", self.l_offshellW_eta, "l_offshellW_eta/D"
        )
        self.hmetree.Branch(
            "l_offshellW_phi", self.l_offshellW_phi, "l_offshellW_phi/D"
        )
        self.hmetree.Branch("l_offshellW_pt", self.l_offshellW_pt, "l_offshellW_pt/D")
        self.hmetree.Branch(
            "l_offshellW_energy", self.l_offshellW_energy, "l_offshellW_energy/D"
        )
        self.hmetree.Branch(
            "nu_onshellW_eta", self.nu_onshellW_eta, "nu_onshellW_eta/D"
        )
        self.hmetree.Branch(
            "nu_onshellW_phi", self.nu_onshellW_phi, "nu_onshellW_phi/D"
        )
        self.hmetree.Branch("nu_onshellW_pt", self.nu_onshellW_pt, "nu_onshellW_pt/D")
        self.hmetree.Branch(
            "nu_onshellW_energy", self.nu_onshellW_energy, "nu_onshellW_energy/D"
        )
        self.hmetree.Branch(
            "nu_offshellW_eta", self.nu_offshellW_eta, "nu_offshellW_eta/D"
        )
        self.hmetree.Branch(
            "nu_offshellW_phi", self.nu_offshellW_phi, "nu_offshellW_phi/D"
        )
        self.hmetree.Branch(
            "nu_offshellW_pt", self.nu_offshellW_pt, "nu_offshellW_pt/D"
        )
        self.hmetree.Branch(
            "nu_offshellW_energy", self.nu_offshellW_energy, "nu_offshellW_energy/D"
        )
        ...
        self.hmetree.Branch(
            "h2tohh_mass_total", self.h2tohh_mass_total, "h2tohh_mass_total/D"
        )
    ...
    def setMETResolution(self, x):
        ...
        print(
            "Setting the MET resolution to be: ", x, " the default is ", self.met_sigma
        )
        self.met_sigma = x
    ...
    def setMETCovMatrix(self, covxx, covyy, covxy, covcorrection):
        ...
        print(
            "Setting MET CovMatrix to correct MET: covxx ",
            covxx,
            " covxy ",
            covxy,
            " covyy ",
    ...
    def showKinematic(self):
        print("lepton1 ")
        self.lepton1_p4.Print()
        print("lepton2 ")
        self.lepton2_p4.Print()
    ...
    def initHMETree(self):
        ...
        self.l_onshellW_eta[0] = -9.0
        self.l_onshellW_phi[0] = -9.0
        self.l_onshellW_pt[0] = -1.0
        self.l_onshellW_energy[0] = -1.0
        self.l_offshellW_eta[0] = -9.0
        self.l_offshellW_phi[0] = -9.0
        self.l_offshellW_pt[0] = -1.0
        self.l_offshellW_energy[0] = -1.0
        self.nu_onshellW_eta[0] = -9.0
        self.nu_onshellW_phi[0] = -9.0
        self.nu_onshellW_pt[0] = -1.0
        self.nu_onshellW_energy[0] = -1.0
        self.nu_offshellW_eta[0] = -9.0
        self.nu_offshellW_phi[0] = -9.0
        self.nu_offshellW_pt[0] = -1.0
        self.nu_offshellW_energy[0] = -1.0
        self.onshellW_eta[0] = -9.0
        self.onshellW_phi[0] = -9.0
        self.onshellW_pt[0] = -1.0
        self.onshellW_energy[0] = -1.0
        self.onshellW_mass[0] = -1.0
        self.offshellW_eta[0] = -9.0
        self.offshellW_phi[0] = -9.0
        self.offshellW_pt[0] = -1.0
        self.offshellW_energy[0] = -1.0
        self.offshellW_mass[0] = -1.0
        self.htoWW_eta[0] = -9.0
        self.htoWW_phi[0] = -9.0
        self.htoWW_pt[0] = -1.0
        self.htoWW_energy[0] = -1.0
        self.htoWW_mass[0] = -1.0
        self.b1jet_eta[0] = -9.0
        self.b1jet_phi[0] = -9.0
        self.b1jet_pt[0] = -1.0
        self.b1jet_energy[0] = -1.0
        self.b2jet_eta[0] = -9.0
        self.b2jet_phi[0] = -9.0
        self.b2jet_pt[0] = -1.0
        self.b2jet_energy[0] = -1.0
        self.htoBB_eta[0] = -9.0
        self.htoBB_phi[0] = -9.0
        self.htoBB_pt[0] = -1.0
        self.htoBB_energy[0] = -1.0
        self.htoBB_mass[0] = -1.0
        self.h2tohh_eta[0] = -9.0
        self.h2tohh_phi[0] = -9.0
        self.h2tohh_pt[0] = -1.0
        self.h2tohh_energy[0] = -1.0
        self.h2tohh_mass[0] = -1.0
        self.h2tohh_mass_total[0] = 0.0
        self.met_pt[0] = -1.0
        self.met_px[0] = -99999.0
        self.met_py[0] = -99999.0
        self.met_phi[0] = -99999.0
        self.weight[0] = 1.0
        self.weight1[0] = 1.0
        self.weight2[0] = 1.0
        self.weight3[0] = 1.0
    ...
    def metCorrection_dijet(self):
        if self.dijetrescalefactor < 0.0:
            print(
                "dibjet correction is working properly, self.dijetrescalefactor ",
                self.dijetrescalefactor,
            )
    ...
    def metCorrection(self):
        if self.b1rescalefactor[0] < 0.0 or self.b2rescalefactor[0] < 0.0:
            print(
                "bjet elaborate correction is working properly, b1rescalefactor ",
                self.b1rescalefactor[0],
                " b2rescalefactor ",
                self.b2rescalefactor[0],
            )
            return ROOT.TVector2(0.0, 0.0)
        metpx_tmp = (
            -(self.b1rescalefactor[0] - 1.0) * self.b1jet_p4.Px()
            - (self.b2rescalefactor[0] - 1.0) * self.b2jet_p4.Px()
        )
        metpy_tmp = (
            -(self.b1rescalefactor[0] - 1.0) * self.b1jet_p4.Py()
            - (self.b2rescalefactor[0] - 1.0) * self.b2jet_p4.Py()
        )
        return ROOT.TVector2(metpx_tmp, metpy_tmp)
    ...
    def nuP4FromOffshellW(
        self, met, lepton1_p4, lepton2_p4, nu1_p4, nu2_p4, case, hMass
    ):
        ...
        tmp_p4_v2 = ROOT.TLorentzVector(
            sqrt(pow(tmp_p4.Pt(), 2) + pow(tmp_p4.M(), 2)),
            0,
            tmp_p4.Pz(),
            tmp_p4.Energy(),
        )
        chdeta = (
            pow(hMass, 2)
            + 2 * (nu_pxpy.Px() * tmp_p4.Px() + nu_pxpy.Py() * tmp_p4.Py())
            - pow(tmp_p4.M(), 2)
        ) / (2.0 * tmp_p4_v2.Pt() * tmp_nu_pt)
        if self.debug:
            print("chdeta = ", chdeta)
        if chdeta < 1.0:
    ...
    def runHME(self):
        ...
        self.met_px[0] = self.met.Px()
        self.met_py[0] = self.met.Py()
        it = 0
        genRandom = ROOT.TRandom3(0)
        boosted_flg = (
            self.b1jet_p4 == None and self.b2jet_p4 == None and self.dijet_p4 != None
        )
        if not self.recobjetrescalec1pdf_flag and not boosted_flg:
            if self.debug:
                print(
                    "self.recobjetrescalec1pdf_flag is False! and not boosted topology"
                )
            self.met_sigma = 0.0
        ...
        while it < self.iterations:
            ...
            rand01 = genRandom.Uniform(0.0, 1.0)
            ...
            if self.met_covcorrection:
                dxy_covcorrection = self.metSmearing_Cov(
                    self.met_px[0],
                    self.met_py[0],
                    self.met_covxx,
                    self.met_covyy,
            ...
            if boosted_flg:
                ...
                met_corr = (
                    self.met
                    + ROOT.TVector2(met_dpx, met_dpy)
                    + self.metCorrection_dijet()
                )
                ...
                if self.recobjetrescalec1pdf_flag:
                    ...
                    if not found_bjetscorr:
                        if self.debug:
                            print(
                                "Warning! no bject correction is found! ignore this iteration, it ",
                                it,
                                " total trials ",
                                ibjetcorr,
                            )
                    ...
                    self.b2rescalefactor[0] = 1.0
                self.htoBB_p4 = (
                    self.b1jet_p4 * self.b1rescalefactor[0]
                    + self.b2jet_p4 * self.b2rescalefactor[0]
                )
            ...
            while isolution < len(solutions):
                ...
                nu_onshellW_pt_tmp = self.nuPtFromOnshellW(
                    self.eta_gen[0],
                    self.phi_gen[0],
                    self.lepton_onshellW_p4,
                    self.wmass_gen[0],
                )
                nu_onshellW_p4_tmp = ROOT.TLorentzVector()
                nu_onshellW_p4_tmp.SetPtEtaPhiM(
                    nu_onshellW_pt_tmp, self.eta_gen[0], self.phi_gen[0], 0
                )
                nu_offshellW_p4_tmp = ROOT.TLorentzVector()
                solutions[isolution] = self.nuP4FromOffshellW(
                    met_corr,
                    self.lepton_onshellW_p4,
                    self.lepton_offshellW_p4,
                    nu_onshellW_p4_tmp,
                    nu_offshellW_p4_tmp,
                    isolution % 2,
                    self.hmass_gen[0],
            ...
            if self.nsolutions[0] == 0:  # no solution in this iterations
                if self.debug:
                    print(" no solution in this iterations ")
                continue
            ...
            while isolution < len(solutions):
                ...
                case = isolution / 2
                self.assignMuP4(case)
                self.nu_onshellW_pt[0] = self.nuPtFromOnshellW(
                    self.eta_gen[0],
                    self.phi_gen[0],
                    self.lepton_onshellW_p4,
                    self.wmass_gen[0],
                )
                self.nu_onshellW_p4.SetPtEtaPhiM(
                    self.nu_onshellW_pt[0], self.eta_gen[0], self.phi_gen[0], 0
                )
                self.nuP4FromOffshellW(
                    met_corr,
                    self.lepton_onshellW_p4,
                    self.lepton_offshellW_p4,
                    self.nu_onshellW_p4,
                    self.nu_offshellW_p4,
                    isolution % 2,
                    self.hmass_gen[0],
                )
                ...
                if self.debug:
                    print(
                        "nu1 p4 ",
                        self.nu_onshellW_p4.Pt(),
                        self.nu_onshellW_p4.Eta(),
                        self.nu_onshellW_p4.Phi(),
                        self.nu_onshellW_p4.Energy(),
                    )
                if self.debug:
                    print(
                        "nu2 p4 ",
                        self.offshellW_p4.Pt(),
                        self.offshellW_p4.Eta(),
                        self.offshellW_p4.Phi(),
                        self.offshellW_p4.Energy(),
                    )
                ...
                if (
                    self.offshellW_mass[0] > self.htoWW_mass[0] / 2.0
                    or self.offshellW_mass[0] > self.onshellW_mass[0]
                ):
                ...
                if self.h2tohh_p4.Pt() / self.h2tohh_p4.E() < 0.00000001:
                    print(
                        "Strange case: h2tohh pt ",
                        self.h2tohh_p4.Pt(),
                        " energy ",
                        self.h2tohh_p4.E(),
                    )
                    self.h2tohh_eta[0] = 1000000.0
                    self.h2tohh_phi[0] = 0.0
                else:
                    self.h2tohh_eta[0] = self.h2tohh_p4.Eta()
                    self.h2tohh_phi[0] = self.h2tohh_p4.Phi()
                ...
                jj = 0
                while ii < isolution:
                    if fabs(self.h2tohh_mass[0] - hme_values[ii]) <= hme_binwidth:
                        N_solution_withinOnebin += 1
                    ii += 1
                trueweight_this = 1.0 / (N_solution_withinOnebin + 1.0)
                while jj <= isolution:
                    if fabs(self.h2tohh_mass[0] - hme_values[jj]) <= hme_binwidth:
                        trueweights[jj] = trueweight_this
                    jj += 1
                ...
                self.hme_h2MassAndoffshellWmass.Fill(
                    self.h2tohh_mass[0], self.offshellW_mass[0], self.weight[0]
                )

                ##weight1 results
                self.weight1[0] = self.weight[0] * self.getWeightFromHist(
                    self.onshellnuptpdf, self.nu_onshellW_pt[0]
                )
                self.hme_h2MassWeight1.Fill(self.h2tohh_mass[0], self.weight1[0])
                self.hme_h2MassAndoffshellWmass_weight1.Fill(
                    self.h2tohh_mass[0], self.offshellW_mass[0], self.weight1[0]
                )

                ##weight2 results
                self.weight2[0] = self.weight[0] * self.getWeightFromHist(
                    self.offshellWmasspdf, self.offshellW_mass[0]
                )
                self.hme_h2MassWeight2.Fill(self.h2tohh_mass[0], self.weight2[0])
                self.hme_offshellWmass.Fill(self.offshellW_mass[0])
                self.hme_h2MassAndoffshellWmass_weight2.Fill(
                    self.h2tohh_mass[0], self.offshellW_mass[0], self.weight2[0]
                )

                self.leptonNuPair[0] = self.correctmunupair
                if self.correctmunupair:
                    self.hme_h2Mass_correctmunupair.Fill(
                        self.h2tohh_mass[0], self.weight[0]
                    )
                    self.hme_h2MassAndoffshellWmass_correctmunupair.Fill(
                        self.h2tohh_mass[0], self.offshellW_mass[0], self.weight[0]
                    )
                else:
                    self.hme_h2Mass_incorrectmunupair.Fill(
                        self.h2tohh_mass[0], self.weight[0]
                    )
            ...
            if self.debug:
                print("it/sols", it, htoWW_vals)
            self.hme_h2Mass_divSolutions.Fill(
                self.h2tohh_mass_total[0] / self.nsolutions[0]
            )
